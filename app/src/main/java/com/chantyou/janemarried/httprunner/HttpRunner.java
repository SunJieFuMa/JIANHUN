...
public abstract class HttpRunner extends AsynEvent {
    ...
    public static final String TAG = HttpRunner.class.getSimpleName();
...
//                    .setTimeOut(10000, 10000);
    ...
    public HttpRunner(int eventCode, String url, Object... params) {
    ...
    protected boolean verifySuccess(Map<String, Object> map) {
        return map != null && map.containsKey("return_code") && JsonUtil.getItemInt(map, "return_code") == 0;
    }
    ...
    protected LinkedHashMap<String, String> getPublicPair() {
        ...
        if (!TextUtils.isEmpty(accessToken)) {
            pList.put("access_token", accessToken);
    ...
    protected LinkedList<NameValuePair> postPublicPair() {
        ...
        String accessToken = AppAndroid.getAccessToken();
        if (!TextUtils.isEmpty(accessToken)) {
            pList.add(new NameValuePair("access_token", accessToken));
    ...
    protected MultipartBody postPublicMultiPair() {
        ...
        String accessToken = AppAndroid.getAccessToken();
        if (!TextUtils.isEmpty(accessToken)) {
            body.addPart(new StringPart("access_token", accessToken));
            log("access_token=" + accessToken);
        }
