...
public class PoiAroundSearchActivity extends Activity implements OnClickListener,
		...
		OnPoiSearchListener,XLocationManager.OnGetLocationListener {
	...
	private LatLng latlngA;//当前用户的坐标点
	private LatLng latlngB;//搜索出来的第一个位置的坐标点
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//禁止软键盘自动弹出，还要在manifest文件中设置这个activity的android:windowSoftInputMode="stateHidden|stateUnchanged"
		getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
		initView();
		//先进行定位以确定LatLonPoint，实现一个自定义接口XLocationManager，跟城市选择界面的一样
	...
	private void initView() {
		...
		mSearchText.setText(getIntent().getStringExtra("address"));
	}
...
//			Toast.makeText(this, "您现在所在的位置是："+location.getAddress(), Toast.LENGTH_SHORT).show();
			latlngA = new LatLng(location.getLatitude(), location.getLongitude());//用户当前的坐标位置
		}
		init();
	}
	...
	public void onPoiSearched(PoiResult result, int rcode) {
		if (rcode == AMapException.CODE_AMAP_SUCCESS) {
			if (result != null && result.getQuery() != null) {// 搜索poi的结果
				...
				ToastUtil.show(PoiAroundSearchActivity.this, "对不起，没有搜索到相关数据！");
			}
		} else  {
...
//				marker.showInfoWindow();//加上这句不知道为什么只有第3个能显示，别的不显示
	            mPoiMarks.add(marker);
				if (i==0){//获取搜索出来的第一个地点的坐标
					latlngB = mPoiMarks.get(0).getPosition();//坐标
					float distance = AMapUtils.calculateLineDistance(latlngA,latlngB);//计算两坐标之间的距离，单位米
					if (distance<1000){
						Toast.makeText(PoiAroundSearchActivity.this,
								"商家地址距离您现在的位置大约为"+distance+"米", Toast.LENGTH_LONG).show();
					}else if (distance>=1000){
						distance=distance/1000;
						Toast.makeText(PoiAroundSearchActivity.this,
								"商家地址距离您现在的位置大约为"+distance+"公里", Toast.LENGTH_LONG).show();
					}
				}
	        }
	    }
